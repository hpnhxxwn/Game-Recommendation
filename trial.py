...
def trainALS(test, validation, training):
    ...
    for i in range(0, len(rank)):
        temp_model = ALS.train(training, rank[i], 5, 0.01)
...
def loadRatings(ratingsFile, spark_context):
    ...
    #print ("Miao Miao")
    #print (f.take(1))
    ratings = f.map(lambda x: parseRating(x)).filter(lambda y: y[1][2] > 0)
...
def __count_and_average_ratings(ratings):
    """Updates the movies ratings counts from 
    the current data self.ratings_RDD
    """
    logger.info("Counting movie ratings...")
    games_ID_with_ratings_RDD = ratings.map(lambda x: (x[1], x[2])).groupByKey()
...
def add_ratings(new_ratings):
    ...
    ratings = ratings.union(new_ratings_RDD)
    
    training, validationOld, testOld = ratings.randomSplit([6.0, 2.0, 2.0], 24)
...
def __predict_ratings(user_and_game_RDD):
...
def get_top_ratings(user_id, games_count):
    """Recommends up to movies_count top unrated movies to user_id
    """
    # Get pairs of (userID, movieID) for user_id unrated movies
    user_unrated_game_RDD = games.filter(lambda rating: not rating[1]==user_id).map(lambda x: (user_id, x[0]))
    # Get predicted ratings
    ratings = __predict_ratings(user_unrated_game_RDD).filter(lambda r: r[2]>=25).takeOrdered(games_count, key=lambda x: -x[1])
...
def get_ratings_for_game_ids(user_id, game_ids):
    ...
    # Get predicted ratings
    ratings = __predict_ratings(requested_games_RDD).collect()
...
class RecommendationEngine:
    def __init__(self, sc):
        ...
        #myRatings = sc.textFile("/home/ubuntu/Game_Recommendation/gamesRecommenderSpark/pythonGames/data/gamelens/personalRatings.txt") \
        #            .map(lambda x: parseRating(x))
        #print ("Wang!!!")
        #print (myRatings.take(1))
        #myRatingsRDD = sc.parallelize(myRatings, 2)

        # load ratings and game titles

        #gameLensHomeDir = sys.argv[1]
